...
int cargaClientesEnArreglo(stCliente arrClientes[], int v, int dim, char archivoDeudores[], char archivoAcreedores[]);
...
int main()
{
    ...
    vArregloClientes = cargaClientesEnArreglo(arregloClientes,vArregloClientes,50, AR_ACREDORES, AR_DEUDORES);
    printf("Clientes en el arreglo\n");
    ...
    printf("La suma de los saldos de los clientes en el arreglo es %f \n",suma);
...
int existeClienteEnArchivo(char nombreArchivo[],int idClienteBuscar)
{
    ...
    if(archi)
    {
        ...
        {
            if(aux.idCliente == idClienteBuscar)
                flag = 1;
        }
        fclose(archi);
    }
...
stCliente cargaUnCliente()
{
    ...
    while(!validaEdad(18,120,cliente.edad));


    return cliente;
}

/** \brief Gonzalo Rochet -
              *
              * \param stCliente[] - Recibe un arreglo de clientes
              * \param
              * \return nuevos validos del arreglo.
              *
              */
int cargaClientesEnArreglo(stCliente arrClientes[], int v, int dim, char archivoDeudores[], char archivoAcreedores[])
{
    ...
    while(v < dim && option != 27 )
    {
        ...
        do
        {
            ...
            if(flag == 0)
            {
                flag = existeClienteEnArchivo(archivoAcreedores,idCliente);

                if(flag == 0)
                {
                    flag = existeClienteEnArchivo(archivoDeudores,idCliente);
...
void balanceTotalBanco(char nombreArchivoDeu[], char nombreArchivoAcr[])
{
    ...
    float balance = sumaSaldosArchivo(nombreArchivoAcr) + sumaSaldosArchivo(nombreArchivoDeu);
